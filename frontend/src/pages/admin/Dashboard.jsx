      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20 }}>
            <StatCard label="Total Products" value={data?.stats.total_products} emoji="👗" color="#d4af37" />
            <StatCard label="Brand Lines" value={data?.stats.total_brands} emoji="🏷️" color="#4f6ef7" />
            <StatCard label="Categories" value={data?.stats.total_categories} emoji="🗂️" color="#22c55e" />
            <StatCard label="Unread Messages" value={data?.stats.unread_messages} emoji="✉️" color="#ef4444" />
            <StatCard label="Total Users" value={data?.stats.total_users} emoji="👥" color="#a78bfa" />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 32 }}>
            ...
            <div className="card" style={{ padding: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                ...
                <Link to="/admin/contacts" style={{ fontSize: '0.78rem', color: '#d4af37' }}>View Inbox →</Link>
              </div>
            ...
            <div className="card" style={{ padding: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                ...
                <Link to="/admin/administration" style={{ fontSize: '0.78rem', color: '#d4af37' }}>Manage Staff →</Link>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
